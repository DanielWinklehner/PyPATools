...
Requires openPMD-beamphysics for the openPMD cases. The OPAL H5hut case only
runs if the reference file from the IsoDAR runs is present on this machine.
"""
import os
import tempfile
import warnings
...
def test_opal_h5_load():
    if not os.path.exists(OPAL_REFERENCE_H5):
        print("[skip] OPAL reference file not present")
        return
